...
```
`DEBUG_AI=1` prints every prompt and raw model response. There is no server test suite; `src/scripts/test_encryption.ts` is a manual script.
...
- `build_exe.bat` — `client build` → copy `client/dist` → `server/public` → `server build` → `server package` → `server/dist/TwitchWatcher.exe`.
...
### Data files
`settings.json` and `users.json` live in `DATA_DIR` (`src/paths.ts`): next to the exe under `pkg`, otherwise the server package root (found by walking up to `package.json`, so it works for ts-node, `tsc` output and the esbuild bundle). Both are gitignored. `users.json` writes are debounced (1 s) and flushed on shutdown.

### Settings are the single source of truth
`src/store/settings.ts` owns all config: `platforms.{twitch,youtube,tiktok}` (each with `enabled` + credentials/tokens), `moderation` (sensitivity, category toggles, skipTrustedRoles), `ai`, `aiLanguage`, `defaultTimeoutDuration`, `isSetupComplete`. Persisted AES-256-GCM with a key derived from `hostname + OS username`; decrypt failure resets to defaults. `load()` migrates the pre-2.0 Twitch-only layout (`{ twitch: {...} }` → `platforms.twitch`) and fills missing keys from defaults. Setup can complete with **zero** platforms enabled; platforms are added later in Settings, and `PUT /api/settings` reconnects any platform whose config changed.
...
- `youtube.ts` — reads chat via **InnerTube** (`youtubei.js`, no key/quota) after resolving the channel's `/live` URL (or `videoIdOverride`); moderates via the **Data API** `liveChatBans` with a Google OAuth token (`services/googleAuth.ts`, scope `youtube.force-ssl`). `unban` only works for bans issued this session (needs the ban id). Custom emoji runs render as `:shortcut:`.
...
- The prompt (`promptBuilder.ts`) is tuned for small models: it lists explicit "ARE / are NOT violations" examples. `gemma3:4b` still over-weights repetition and the literal word "hate"; adjust examples rather than adding more CRITICAL-style instructions, which made it paranoid.
